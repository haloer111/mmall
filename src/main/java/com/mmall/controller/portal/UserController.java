...
//            session.setAttribute(Const.CURRENT_USER, user.getData());
            ...
            RedisShardedPoolUtil.setEx(session.getId(), JsonUtil.obj2String(user.getData()), Const.RedisCacheExtime
...
//        User currentUser = (User) session.getAttribute(Const.CURRENT_USER);
        ...
        if (response.isSuccess()) {
            ...
            RedisShardedPoolUtil.setEx(loginToken, JsonUtil.obj2String(response.getData()), Const.RedisCacheExtime
                    .REDIS_SESSION_EXTIME);
        }
        return response;
    }
...
//        User currentUser = (User) session.getAttribute(Const.CURRENT_USER);
        ...
        User currentUser = JsonUtil.string2Obj(userJsonStr, User.class);
        if (currentUser == null) {
            return ServerResponse.createByError(ResponseCode.NEED_LOGIN.getCode(), "用户未登录，需要强制登录");
